...
export function resolveForVersion(version, platformID, opts) {
  ...
  // Prioritized the wanted extension in the file prefs
  if (_opts.wanted) _opts.filePreference.unshift(_opts.wanted);

  // Normalize platform id
  platformID = detectPlatform(platformID);

  // x64 and arm64 could get a universal download
  const universalCompatible = platformID.includes("osx") && !platformID.includes("universal");

  return version.platforms
    .filter(function (pl) {
      if (_opts.preferUniversal && universalCompatible && pl.type.includes("universal")) {
        return true;
      }
